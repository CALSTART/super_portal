...
def hvip():
    ...
    m_d_truck_list = ['Delivery Truck', 'Truck - Straight', 'Truck - Box', 'Parcel Delivery', 'Truck - Medium Duty', 'MH']
    ...
    for index, row in data.iterrows():
        if ((row['GVWR'] == '26,001 - 33,000') or (row['GVWR'] == '33,001 and up')):
            ...
            if row['Segment'] in m_d_truck_list:
                print('i hit this statement')
...
def rsbpp_buses():
    ...
    rsbpp_2.rename(columns={"Applicant": "Entity", "New Bus Delivery Date":"Date Delivered", "New Bus Manufacturer":"Manufacturer", "New Bus VIN":"VIN", "Technology Type":"Fuel Type", "Total Project Cost":"Funding Total", "Local Air District":"Air District"}, inplace=True)
    ...
    rsbpp_2 = remove_ad_suffix(rsbpp_2)
...
def sacramento():
    ...
    for index, row in sac_df.iterrows():
        if (row['GVWR'] >= '14,001 - 16,000#'):
            data.loc[index, 'GVWR'] = '14001-19500'
        elif (row['GVWR'] == '33,000# +'):
                data.loc[index, 'GVWR'] = '33001 and up'
...
#6 - CMIS
def cmis():
    cmis_df = pd.read_csv('CMIS.csv')
    cmis_df.rename(columns={"School fleet owner": "Entity", "New Bus Manufacturer":"Manufacturer","New Bus Model Year":"Date Delivered", "Total Project Funding":"Funding Total", "New Bus VIN":"VIN", "Local Air District":"Air District"}, inplace=True)
    cmis_df['Source'] = 'CMIS'
    cmis_df['Fuel Type'] = 'ZEV'
    cmis_df['Segment'] = 'School bus'
    cmis_df['Address'] = cmis_df['Bus Storage Street'].astype(str) + " " + cmis_df['Bus Storage City'].astype(str) + ", " + "CA " + cmis_df['Bus Storage Zip'].astype(str)
    cmis_df['County'] = 'Sacramento'
    cmis_df['DAC?'] = 'Yes'
    cmis_df['Latitude'] = ''
    cmis_df['Longitude'] = ''
    cmis_df['GVWR'] = ''
    list_of_zips = cmis_df['Bus Storage Zip'].to_list()
    cmis_df = get_air_district_county(list_of_zips, cmis_df)
    cmis_df= cmis_df[['Source', 'VIN','Entity','Date Delivered','Manufacturer','Segment', 'Fuel Type', 'GVWR', 'Funding Total', 'Air District', 'County', 'DAC?', 'Address', 'Latitude', 'Longitude']]
...
# def dmv_ihs():
#     data = pd.read_csv('dmv_ihs.csv')
#     data.rename(columns={"REG_NAME": "Entity", "VEHICLE_TYPE":"Segment", "FUEL":"Fuel Type"}, inplace=True)
#     data['Date Delivered'] = ""
#     data
#     #filter out state
#     #adjust vehicle type label
#     #adjust fuel type label
#     data["Source"] = "DMV_IHS"
#     data= data[['Source', 'VIN', 'Entity','Date Delivered', 'Manufacturer','Segment', 'Fuel Type','Funding Total', 'Air District', 'County', 'DAC?', 'Address', 'Latitude', 'Longitude']]
#     return data
...
def standardize_segment(data):
    ...
    "Specialty Vehicle":"Other", "Utility Truck":"Other", "Delivery Truck":"MD Truck", "Refuse Truck":"Refuse Truck","Low-floor":"Transit Bus", "Coach Bus":"Coach",
    "Bus - Medium Duty":"Shuttle Bus", "Special Needs":"Shuttle Bus","School Bus C":"School Bus", "School Bus D":"School Bus",
    "School Bus A":"School Bus", "Transit Bus":"Transit Bus", "School Bus":"School Bus", "Cargo Van":"Cargo Van", "Other": "Other",
    "Shuttle Bus":"Shuttle Bus", "HH":"HD Truck", "MH":"MD Truck", "SB":"School bus", "SW":"Other", "TV":"Transit bus", "UB":"Transit bus",
     "MD":"MD Truck"}
...
def standardize_gvwr(data):
    #gvwr_ranges = [10,001-14,000, 14001-19500, 19501-26000,26001-33000, '33001 and up']
    for index, row in data.iterrows():
        if (row['GVWR'] >= '10001') and (row['GVWR'] <= '14000'):
...
if __name__ == "__main__":
    ...
    data_joined_1 = pd.concat([hvip_data, vw_data, cap_moyer_df, ict_df, cmis_df, prop1b], ignore_index = True)

    #clean segment
    data_joined_1_segment_clean = standardize_segment(data_joined_1)
    data_joined_gvwr_clean = standardize_gvwr(data_joined_1_segment_clean)
    print(len(data_joined_gvwr_clean))
    #join hvip after - bc it already has cleaned segment
    #data_joined_2 = pd.concat([data_joined_gvwr_clean], ignore_index = True)
    #remove air district suffix
    data_joined_ad_clean = remove_ad_suffix(data_joined_gvwr_clean)
    ...
    final_df.to_csv('super_portal_data_v1.32.csv')
